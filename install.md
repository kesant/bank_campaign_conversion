# Bank_Campaign_Conversion guide installation
...
conda activate bank_campaign_conversion
```
...
mamba activate bank_campaign_conversion
```
or 
...
To move beyond notebook prototyping, all reusable code should go into the `Bank_Campaign_Conversion/` folder package. To use that package inside your project, install the project's module in editable mode, so you can edit files in the `Bank_Campaign_Conversion` folder and use the modules inside your notebooks :
...
```
This will install the project module in "editable" mode so that any changes you make to the code in the Bank_Campaign_Conversion/ folder will be reflected immediately without needing to reinstall.
...
```python
from Bank_Campaign_Conversion.utils.paths import data_dir
data_dir()
```
